...
public class Nametags extends Module {
    ...
    public static String getHealthString(LivingEntityRenderState state, HealthRenderState hrs, float health,
            float maxHealth) {
        String ret = "";
    ...
    public static String getNametagString(LivingEntityRenderState state) {
        ...
        if (!healthDisplay.value)
            return state.customName.getString(); // name only
        ...
        if (type == HealthRenderState.EntityType.PASSIVE && !passive.value)
            return null;
        if (type == HealthRenderState.EntityType.HOSTILE && !hostile.value)
            return null;
        if (type == HealthRenderState.EntityType.PLAYER && !players.value)
            return null;
        if (type == HealthRenderState.EntityType.OTHER)
            return null;
        ...
        String ret = color + obf + state.customName.getString() + reset + " " + hpColor
                + getHealthString(state, hrs, health, maxHealth) + heart;
