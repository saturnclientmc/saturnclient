...
public class ConfigManager {
    private static File configFile = new File(MinecraftClient.getInstance().runDirectory, "saturn.json");
    ...
    public ConfigManager(String namespace) {
        ...
        properties.put(namespace, new HashMap<>());
    }
    ...
    public static void load() {
        try {
            ...
            JsonObject jsonObject = JsonParser.parseString(new String(Files.readAllBytes(configFile.toPath())))
                    .getAsJsonObject();
            ...
            for (String namespace : properties.keySet()) {
                ...
                if (config == null) {
                    SaturnClient.LOGGER.warn("No config found for namespace: " + namespace);
                    continue;
                }
                ...
                for (String propertyName : propertyMap.keySet()) {
                    ...
                    if (c == null) {
                        SaturnClient.LOGGER.warn("No property found in config for " + propertyName);
                        continue;
                    }

                    SaturnClient.LOGGER.info("got prop " + c);
                    Property<?> p = propertyMap.get(propertyName);
                    SaturnClient.LOGGER.info("setting prop " + propertyName + " to " + p.value);
                    ...
                    if (p.matchesJson(c)) {
                        ...
                        SaturnClient.LOGGER.warn("Property does not match JSON: " + propertyName);
                    }
                }
            }
        } catch (IOException e) {
                ...
                for (String propertyName : propertyMap.keySet()) {
                    ...
                    switch (property.getType()) {
                        ...
                        case STRING:
                            ...
                            break;
                        default:
                            SaturnClient.LOGGER.warn("Unknown property type for: " + propertyName);
                            break;
                    }
