...
public class ColorDialog extends SaturnScreen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        // Draw the HSV hue/saturation square (brightness fixed at 1)
        for (int x = 0; x < PICKER_WIDTH; x++) {
            for (int y = 0; y < PICKER_HEIGHT; y++) {
                float hue = x / (float) PICKER_WIDTH;
                float saturation = y / (float) PICKER_HEIGHT;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (relX >= 0 && relX < PICKER_WIDTH && relY >= 0 && relY < PICKER_HEIGHT) {
            float hue = relX / (float) PICKER_WIDTH;
            float saturation = relY / (float) PICKER_HEIGHT;
            prop.value = Color.HSBtoRGB(hue, saturation, 1.0f);
